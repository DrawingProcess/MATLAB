...
for i = 1:numParticles
  ...
  newParticles(i).pose = particles(idx).pose;
  newParticles(i).history = particles(idx).history;
  
  for l = 1:size(particles(idx).landmarks,2) % initialize the landmarks aka the map
    newParticles(i).landmarks(l).observed = particles(idx).landmarks(l).observed;
    newParticles(i).landmarks(l).mu = particles(idx).landmarks(l).mu;       % 2D position of the landmark
    newParticles(i).landmarks(l).sigma = particles(idx).landmarks(l).sigma; % covariance of the landmark
  end
